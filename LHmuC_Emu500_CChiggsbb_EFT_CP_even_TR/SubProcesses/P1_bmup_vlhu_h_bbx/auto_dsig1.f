...
C       LP=SIGN(1,LPP(IB(1)))
     ...
     $       -PP(3,I))))
        ...
        ENDIF
        B1=PDG2PDF(LPP(IB(1)),5, IB(1),XBK(IB(1)), QSCALE)
      ENDIF
...
C       LP=SIGN(1,LPP(IB(2)))
        ...
        ENDIF
        MUP2=PDG2PDF(LPP(IB(2)),-13, IB(2),XBK(IB(2)), QSCALE)
      ENDIF
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! b mu+ > vm~ b b~ u
      PD(IPROC)=B1*MUP2
...
C     
      INTEGER I,ITYPE,LP,IPROC
      DOUBLE PRECISION QSCALE
      DOUBLE PRECISION B1(VECSIZE_MEMMAX)
      DOUBLE PRECISION MUP2(VECSIZE_MEMMAX)
      DOUBLE PRECISION XPQ(-7:7),PD(0:MAXPROC)
...
C     DATA
C     
      DATA B1/VECSIZE_MEMMAX*1D0/
      DATA MUP2/VECSIZE_MEMMAX*1D0/
C     ----------
C     BEGIN CODE
C     ----------
      SELECTED_HEL(:) = 0
     ...
     &    NB_WARP_USED, WARP_SIZE, VECSIZE_USED
      ...
      DO CURR_WARP=1, NB_WARP_USED
        ...
        DO IWARP=1, WARP_SIZE
          ...
          IF (ABS(LPP(IB(1))).GE.1) THEN
C           LP=SIGN(1,LPP(IB(1)))
            B1(IVEC)=PDG2PDF(LPP(IB(1)),5, IB(1),ALL_XBK(IB(1),IVEC)
     $       ,DSQRT(ALL_Q2FACT(1, IVEC)))
          ENDIF
          IF (ABS(LPP(IB(2))).GE.1) THEN
C           LP=SIGN(1,LPP(IB(2)))
            MUP2(IVEC)=PDG2PDF(LPP(IB(2)),-13, IB(2),ALL_XBK(IB(2)
     $       ,IVEC),DSQRT(ALL_Q2FACT(2, IVEC)))
          ENDIF
        ENDDO  ! IWARP LOOP
      ENDDO  ! CURRWARP LOOP
      ALL_PD(0,:) = 0D0
      ...
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=B1(IVEC)*MUP2(IVEC)
