...
# -----------------------------
# Input: LHeC cross sections (pb) from your MG banners
# -----------------------------

sigma_SM = 0.1022315443

# CP-even: sigma(TR) points
TR_pts = np.array([-0.1, -0.01, 0.0, +0.01, +0.1], dtype=float)
sig_TR = np.array([
    0.0735613915,   # TR=-0.1
    0.0998505189,   # TR=-0.01
    0.1022315443,   # TR=0
    0.1044848894,   # TR=+0.01
    0.1206675259,   # TR=+0.1
], dtype=float)

# CP-odd: sigma(TI) points
TI_pts = np.array([-0.1, -0.01, 0.0, +0.01, +0.1], dtype=float)
sig_TI = np.array([
    0.1055448104,   # TI=-0.1
    0.1022392944,   # TI=-0.01
    0.1022315443,   # TI=0
    0.1022664143,   # TI=+0.01
    0.1055466908,   # TI=+0.1
], dtype=float)
...
# -----------------------------
# Helpers: expected counts and Poisson Asimov q(theta)
# -----------------------------
B = L_pb * sigma_SM  # Asimov "data" = SM expectation
...
# -----------------------------
# Simple bisection root finder for q(theta)=q_target
# -----------------------------
...
def find_limits():
    ...
    # TR limits: find one bracket <0 and one >0
...
def print_summary(TR_low, TR_high, TI_lim):
    ...
    print("\n=== LHeC yields at L = 1 ab^-1 (stat-only) ===")
...
def plot_sigma_TR(TR_low, TR_high, out_prefix, save):
    ...
    ax.set_xlabel("TR")
    ax.set_ylabel("σ [pb]")
    ax.set_title(r"LHeC @ 1 ab$^{-1}$: σ(TR) fit and points")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    savefig(fig, out_prefix / "LHeC_sigma_vs_TR", save)
...
def plot_sigma_TI(TI_lim, out_prefix, save):
    ...
    ax.set_xlabel("TI")
    ax.set_ylabel("σ [pb]")
    ax.set_title(r"LHeC @ 1 ab$^{-1}$: σ(TI) fit and points")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    savefig(fig, out_prefix / "LHeC_sigma_vs_TI", save)
...
def plot_q_curves(TR_low, TR_high, TI_lim, out_prefix, save):
    ...
    ax1.set_xlabel("TR")
    ax1.set_ylabel("q")
    ax1.set_title(r"LHeC @ 1 ab$^{-1}$: q(TR)")
    ax1.grid(True, alpha=0.3)
    ax1.legend(fontsize=9)
    fig1.tight_layout()
    savefig(fig1, out_prefix / "LHeC_q_vs_TR", save)
    ...
    ax2.set_xlabel("TI")
    ax2.set_ylabel("q")
    ax2.set_title(r"LHeC @ 1 ab$^{-1}$: q(TI)")
    ax2.grid(True, alpha=0.3)
    ax2.legend(fontsize=9)
    fig2.tight_layout()
    savefig(fig2, out_prefix / "LHeC_q_vs_TI", save)


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--save", action="store_true", help="Save plots to --outdir as PDF+PNG.")
    ap.add_argument("--outdir", default="plots_limits", help="Output directory for plots.")
    args = ap.parse_args()
